...
from apps.survey.services import SubmissionService, SectionService, DiscussionService
...
class DiscussionViewSet(SubmissionAPIBaseViewSet):
    ...
    def _create_classic_survey(
        ...
        chat = self._discussion_service.get_discussion_chat(
            section=section, submission=request.submission, get_or_404=True
        )
        request_serializer = ClassicSurveyMessageRequestSerializer(data=request.data)
        request_serializer.is_valid(raise_exception=True)
        [user_message, assistant_message] = self._discussion_service.create_message(
    ...
    def _create_avatar_survey(
        ...
        request_serializer.is_valid(raise_exception=True)
        message = self._discussion_service.save_message(
            message=request_serializer, chat=chat
        )
    ...
    def close(
        ...
        self._discussion_service.close_chat(chat=chat, save_empty_final_note=True)
    ...
    def prompts(
        ...
        prompts = self._discussion_service.get_prompts(chat=chat)
    ...
    def final_note(
        ...
        request_serializer.is_valid(raise_exception=True)
        chat = self._discussion_service.save_final_note(
            note=request_serializer, chat=chat
        )
        return Response(data=None, status=status.HTTP_204_NO_CONTENT)
